...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (
    ...
    typeof body !== 'object' ||
    !('eventType' in body) ||
    !isAnalyticsEventType(body.eventType) ||
    !('postId' in body) ||
    ...
    return NextResponse.json(INVALID_EVENT_RESPONSE, { status: 400 })
  const channel =
    'metadata' in body &&
    body.metadata &&
    typeof body.metadata === 'object' &&
    'channel' in body.metadata
      ? body.metadata.channel
      : undefined
  const shareChannel = isShareChannel(channel) ? channel : undefined
  if (body.eventType === 'share' && !shareChannel)
    return NextResponse.json(INVALID_EVENT_RESPONSE, { status: 400 })
  const features = await getPublicCommunityFeatures()
  if (
    (body.eventType === 'article_view' && !features.articleViewTracking) ||
    (body.eventType === 'share' && !features.shareTracking)
  )
