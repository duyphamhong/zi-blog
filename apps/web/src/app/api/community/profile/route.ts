...
export async function GET(): Promise<NextResponse> {
  ...
  return NextResponse.json({
    ...
    shortIdentityCode: actor.shortIdentityCode,
    status: actor.status,
  })
}
...
export async function PATCH(request: Request): Promise<NextResponse> {
  ...
  if (!body || typeof body !== 'object')
    return NextResponse.json(INVALID_PROFILE_RESPONSE, { status: 400 })
  const displayName =
    'displayName' in body && typeof body.displayName === 'string'
      ? body.displayName.replace(/\s+/g, ' ').trim()
      : ''
  const avatarCandidate =
    'avatarKey' in body && typeof body.avatarKey === 'string' ? body.avatarKey : undefined
  const avatarKey =
    avatarCandidate && (ANONYMOUS_AVATAR_KEYS as readonly string[]).includes(avatarCandidate)
      ? (avatarCandidate as (typeof ANONYMOUS_AVATAR_KEYS)[number])
      : undefined
  const validName =
  ...
  if (!validName || !validAvatar)
    return NextResponse.json(INVALID_PROFILE_RESPONSE, { status: 400 })
  const actor = await resolveAnonymousActor()
  const payload = await getPayloadClient()
  await payload.update({
    collection: 'anonymous-profiles',
    id: actor.profileId,
    data: { avatarKey, displayName },
    overrideAccess: true,
  })
  return NextResponse.json({
    avatarKey: avatarKey ?? actor.avatarKey,
    displayName,
    shortIdentityCode: actor.shortIdentityCode,
  })
}
