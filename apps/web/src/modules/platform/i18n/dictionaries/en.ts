export const enDictionary = {
  accessibility: {
    close: 'Close',
    currentLanguage: 'Current language: English',
    languageSwitcher: 'Change language',
    loading: 'Loading content',
    mobileNavigation: 'Menu',
    openMenu: 'Open menu',
    primaryNavigation: 'Primary navigation',
    skipToContent: 'Skip to content',
    themeToggle: 'Change color theme',
  },
  common: {
    by: 'By',
    category: 'Category',
    featured: 'Featured',
    next: 'Next',
    page: 'Page',
    previous: 'Previous',
    series: 'Series',
    tag: 'Tag',
    unlisted: 'Unlisted',
  },
  error: {
    description: 'We could not load this page.',
    heading: 'Something went wrong',
    reference: 'Reference',
    tryAgain: 'Try again',
  },
  footer: {
    content: 'Practical engineering notes for Pure Software Engineers.',
    navigation: 'Footer navigation',
    rights: 'All rights reserved.',
  },
  home: {
    browseCategories: 'Browse categories',
    eyebrow: 'Road to Solution Architect',
    featuredPosts: 'Featured posts',
    freshIdeas: 'Fresh ideas',
    heading:
      'Become a true architect through practical engineering stories and fresh perspectives.',
    intro: 'Architecture, tooling, and lessons learned from shipping software.',
    latestPosts: 'Latest posts',
    noPosts: 'Publish a post in Admin and it will appear here. Drafts stay private.',
    postCount: 'published posts',
    primaryCta: 'Explore articles',
    secondaryCta: 'Browse series',
    series: 'In-depth series',
    seriesDescription: 'Structured paths through architecture, tooling, and delivery.',
    topics: 'Explore topics',
    viewSeries: 'View series',
    viewAll: 'View all',
  },
  navigation: {
    architecture: 'Architecture',
    featured: 'Featured',
    posts: 'Posts',
    search: 'Search',
    series: 'Series',
  },
  preview: {
    protectedNotice: 'Protected preview — this content may not be published.',
    title: 'Content preview',
  },
  code: {
    copied: 'Copied',
    copy: 'Copy code',
  },
  notFound: {
    description: 'The content may not exist, may still be a draft, or may have moved.',
    heading: 'Page not found',
    returnHome: 'Return home',
  },
  pagination: {
    label: 'Pagination',
    next: 'Next',
    of: 'of',
    previous: 'Previous',
  },
  post: {
    allDescription: 'Published engineering notes, newest first.',
    allPosts: 'All posts',
    authorEmpty: 'This author has no published posts.',
    authorHeading: 'Posts by',
    categoryEmpty: 'This category has no published posts.',
    minRead: 'min read',
    nextInSeries: 'Next in series',
    noPosts: 'No published posts are available.',
    part: 'Part',
    previousInSeries: 'Previous in series',
    publishedPosts: 'Published posts',
    shareLinkedIn: 'Share on LinkedIn',
    shareX: 'Share on X',
    like: 'Like',
    dislike: 'Dislike',
    reactionError: 'We could not update your reaction.',
    commentsHeading: 'Comments',
    commentsEmpty: 'No published comments yet.',
    commentsError: 'We could not load comments.',
    commentLabel: 'Comment',
    nameLabel: 'Your name',
    commentSubmit: 'Submit comment',
    commentPending:
      'Your comment will appear immediately. Staff can remove inappropriate comments later.',
    engagementHeading: 'Was this article helpful?',
    activeLike: 'Liked',
    share: 'Share',
    shareArticle: 'Share article',
    shareFacebook: 'Facebook',
    copyLink: 'Copy link',
    copiedLink: 'Link copied',
    nativeShare: 'Share with device',
    profileAnonymous: 'Anonymous',
    editProfile: 'Edit profile',
    profileStoredLocally: 'Your anonymous profile is stored only in this browser.',
    profileMenu: 'Guest profile',
    avatarLabel: 'Choose avatar',
    saveProfile: 'Save changes',
    cancel: 'Cancel',
    commentPlaceholder: 'Share your thoughts…',
    commentCount: 'characters',
    commentSubmitted: 'Your comment is now published.',
    beFirstComment: 'Be the first to share your perspective.',
    newest: 'Newest',
    tagEmpty: 'This tag has no published posts.',
    translationUnavailable: 'This article is not available in English yet.',
    seriesEmpty: 'This series has no published posts.',
  },
  search: {
    button: 'Search',
    checkDescription: 'Search queries must contain between 2 and 100 characters.',
    checkTitle: 'Check your search',
    emptyDescription: 'Enter at least two characters to search published public posts.',
    emptyTitle: 'Start a search',
    label: 'Search published posts',
    noResults: 'No results',
    noResultsDescription: 'No published posts matched',
    placeholder: 'Search title or excerpt',
    resultsFor: 'Results for',
    shortcutHint: 'Press Ctrl K from any page to open search.',
    title: 'Search',
  },
  site: {
    defaultSeoDescription: 'Practical engineering notes for Pure Software Engineers.',
    defaultSeoTitle: 'Zi-Blog Technology Notes',
    description: 'Practical engineering notes for Pure Software Engineers.',
    name: 'Zi-Blog',
  },
} as const
